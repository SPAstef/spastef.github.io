...
- Number theory tells us that $\legendre{x}{p} = x^{\frac{p - 1}{2}} \bmod p$.
- Complexity theory related to the [quadratic residuosity problem](https://en.wikipedia.org/wiki/Quadratic_residuosity_problem), 
and existing cryptanalysis, lead us to believe[^2] that correctly predicting the values of the 
sequence $\legendre{x + n}{p}_{n \in \mathbb{N}}$, where $x$ is unknown, [has complexity at least $\call{O}{\sqrt{p}}$](https://eprint.iacr.org/2019/862.pdf).